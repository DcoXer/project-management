...
# Hapus ASSET_URL jika ada, biar asset load relative ke origin (mencegah CORS)
$envContent = $envContent -replace "(?m)^ASSET_URL=.*\r?\n", ""
Set-Content $envPath $envContent -NoNewline
Write-Host "APP_URL diupdate ke $tunnelUrl" -ForegroundColor Green
