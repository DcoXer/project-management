    ...
    <AppLayout>
        ...
        <div v-if="isPmOrAdmin" class="mt-8">
            <div class="flex items-center gap-3 mb-4">
                <h2 class="text-xs font-semibold text-ink-400 dark:text-sage-600 uppercase tracking-widest">Workload Tim</h2>
                <div class="flex-1 h-px bg-ink-100 dark:bg-white/5"></div>
                <span class="text-xs text-ink-300 dark:text-sage-700 font-medium">{{ memberWorkload.length }} member</span>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                <div v-for="member in memberWorkload" :key="member.id"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-1.5 flex-wrap">
                                <p class="text-sm font-semibold text-ink-800 dark:text-sage-200 truncate">{{ member.name }}</p>
                                <span v-if="member.pivot?.role === 'manager'" class="text-[10px] font-semibold text-sage-600 dark:text-sage-400 bg-sage-100 dark:bg-sage-300/10 px-1.5 py-0.5 rounded-md shrink-0">PM</span>
                            </div>
                            <p class="text-xs text-ink-400 dark:text-sage-600 mt-0.5">
                                {{ member.pivot?.role === 'manager' ? 'Project Manager' : specializationLabel(member.pivot?.specialization) }}
                            </p>
                        </div>
                        <!-- Task counter -->
                        <div class="text-right shrink-0">
                            <p class="text-sm font-bold text-ink-900 dark:text-sage-200 leading-none">
                                {{ member.done_count }}<span class="text-ink-300 dark:text-sage-700 font-normal text-xs">/{{ member.task_count }}</span>
                            </p>
                            <p class="text-[10px] text-ink-400 dark:text-sage-600 mt-0.5">selesai</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1 flex-1">
                        <p v-if="!member.tasks.length" class="text-xs text-ink-300 dark:text-sage-700 italic py-1">Belum ada task.</p>
                        <Link v-for="task in member.tasks" :key="task.id" :href="`/tasks/${task.id}`"
                            ...
                            <span class="text-xs text-ink-700 dark:text-sage-300 truncate flex-1 group-hover:text-ink-900 dark:group-hover:text-sage-100 transition">{{ task.title }}</span>
                            <TaskStatusBadge :status="task.status" sm />
                        </Link>
                    </div>
                </div>
                ...
                <div v-if="unassignedTasks.length"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex-1">
                            <p class="text-sm font-semibold text-ink-800 dark:text-sage-200">Belum Ditugaskan</p>
