<template>
    <AppLayout>
        <div class="space-y-5">
            ...
            <div>
                <h1 class="text-2xl font-bold text-ink-900 dark:text-white tracking-tight">Projects</h1>
                <p class="text-sm text-ink-500 dark:text-white mt-1">Semua project yang tersedia</p>
            </div>
            ...
            <div class="bg-white dark:bg-ink-800 rounded-xl border border-ink-900/8 dark:border-sage-300/8 p-4 shadow-sm">
                <div class="flex flex-wrap gap-3">
                    <input
                        ...
                        class="border border-ink-200 dark:border-ink-600 bg-sage-50 dark:bg-ink-850 text-ink-900 dark:text-white placeholder-ink-300 dark:placeholder-ink-500 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ink-900/15 dark:focus:ring-sage-300/20 w-52 transition"
                    />
                    ...
                    <button v-if="hasFilter" @click="resetFilter" class="text-sm text-ink-500 dark:text-white hover:text-ink-800 dark:hover:text-white px-3 py-2 rounded-lg hover:bg-sage-100 dark:hover:bg-sage-300/8 transition">Reset</button>
                </div>
            </div>

            <!-- Empty -->
            <div v-if="projects.data.length === 0" class="bg-white dark:bg-ink-800 rounded-xl border border-ink-900/8 dark:border-sage-300/8 px-5 py-12 text-center text-ink-300 dark:text-white text-sm shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                >
                    <div class="flex items-start justify-between gap-2 mb-3">
                        <h3 class="font-semibold text-ink-900 dark:text-white text-sm leading-snug">{{ project.name }}</h3>
                        <PriorityBadge :priority="project.priority" sm class="shrink-0" />
                    </div>

                    <p v-if="project.description" class="text-xs text-ink-400 dark:text-white mb-3 line-clamp-2">{{ project.description }}</p>

                    <div class="flex items-center gap-2 mb-4">
                        <ProjectStatusBadge :status="project.status" sm />
                        <span class="text-xs text-ink-300 dark:text-white">oleh {{ project.creator.name }}</span>
                    </div>

                    <div>
                        <div class="flex justify-between text-xs text-ink-400 dark:text-white mb-1">
