        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 section-charts">
                ...
                <div class="lg:col-span-2 bg-white dark:bg-ink-800 rounded-xl border border-ink-900/8 dark:border-sage-300/8 p-5 shadow-sm">
                    <h2 class="font-semibold text-ink-900 dark:text-white mb-4">Task Tim by Priority</h2>
            ...
            <!-- Middle Row: Aktivitas + Deadlines -->
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 section-middle">
                ...
                <div class="bg-white dark:bg-ink-800 rounded-xl border border-ink-900/8 dark:border-sage-300/8 shadow-sm">
                    <div class="px-5 py-4 border-b border-ink-100 dark:border-sage-300/8">
                        <h2 class="font-semibold text-ink-900 dark:text-white">Aktivitas Terbaru</h2>
                    </div>
                    <div class="divide-y divide-ink-50 dark:divide-sage-300/5">
                        <div v-if="!recentActivities.length" class="px-5 py-8 text-center text-sm text-ink-300 dark:text-white">Belum ada aktivitas.</div>
                        <div v-for="activity in recentActivities.slice(0, 5)" :key="activity.id" class="flex items-start gap-3 px-5 py-3.5">
                            <div class="shrink-0 w-8 h-8 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center text-xs font-bold text-violet-700 dark:text-violet-300">
                                {{ initials(activity.user?.name) }}
                            </div>
                            <div class="min-w-0 flex-1">
                                <p class="text-sm text-ink-700 dark:text-white leading-snug">{{ activity.description }}</p>
                                <p class="text-xs text-ink-400 dark:text-white mt-0.5">{{ timeAgo(activity.created_at) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-ink-800 rounded-xl border border-ink-900/8 dark:border-sage-300/8 shadow-sm">
                    ...
                    <div class="divide-y divide-ink-50 dark:divide-sage-300/5">
                        <div v-if="!upcomingDeadlines.length" class="px-5 py-8 text-center text-sm text-ink-300 dark:text-white">Tidak ada deadline mendekat.</div>
                        <Link v-for="task in upcomingDeadlines.slice(0, 5)" :key="task.id" :href="`/tasks/${task.id}`"
                              class="flex items-center justify-between gap-2 px-5 py-3.5 hover:bg-sage-50 dark:hover:bg-sage-300/4 transition">
                            <div class="min-w-0">
                                <p class="text-sm font-medium text-ink-900 dark:text-white truncate">{{ task.title }}</p>
                                <p class="text-xs text-ink-400 dark:text-white mt-0.5">{{ task.project?.name }} · {{ formatDate(task.due_date) }}</p>
                            </div>
            ...
            <!-- Bottom Row: Projects + Pending Review -->
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 section-bottom">
                ...
                <div class="bg-white dark:bg-ink-800 rounded-xl border border-ink-900/8 dark:border-sage-300/8 shadow-sm">
                    <div class="flex items-center justify-between px-5 py-4 border-b border-ink-100 dark:border-sage-300/8">
                        <h2 class="font-semibold text-ink-900 dark:text-white">Project yang Dikelola</h2>
                ...
                <div class="bg-white dark:bg-ink-800 rounded-xl border border-ink-900/8 dark:border-sage-300/8 shadow-sm">
                    <div class="flex items-center justify-between px-5 py-4 border-b border-ink-100 dark:border-sage-300/8">
                        <h2 class="font-semibold text-ink-900 dark:text-white">Menunggu Review</h2>
                        <Link href="/tasks?status=review" class="text-xs font-medium text-sage-600 dark:text-white hover:opacity-70 transition">Lihat semua →</Link>
                    </div>
                    <div class="divide-y divide-ink-50 dark:divide-sage-300/5">
                        <div v-if="!pendingReviewTasks.length" class="px-5 py-8 text-center text-sm text-ink-300 dark:text-white">Tidak ada task yang menunggu review.</div>
                        <Link v-for="task in pendingReviewTasks" :key="task.id" :href="`/tasks/${task.id}`"
                              class="block px-5 py-4 hover:bg-sage-50 dark:hover:bg-sage-300/4 transition">
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-ink-900 dark:text-white truncate">{{ task.title }}</p>
                                    <p class="text-xs text-ink-400 dark:text-white mt-0.5">{{ task.project.name }} · {{ task.assignee?.name ?? '-' }}</p>
                                </div>
                                <PriorityBadge :priority="task.priority" sm class="shrink-0" />
                            </div>
                            <p v-if="task.due_date" class="text-xs text-ink-400 dark:text-white mt-1.5">Due: {{ formatDate(task.due_date) }}</p>
                        </Link>
                    </div>
                </div>
...
const statCards = computed(() => [
    { label: 'Project Dikelola', value: props.stats?.managed_projects ?? 0, icon: ICONS.folder,    iconBg: 'bg-violet-100 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400',     valueClass: 'text-ink-900 dark:text-white',           accentColor: '#8b5cf6', sparkline: SPARKLINES[0] },
    { label: 'Total Task Tim',   value: props.stats?.total_tasks      ?? 0, icon: ICONS.clipboard, iconBg: 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400',             valueClass: 'text-ink-900 dark:text-white',           accentColor: '#3b82f6', sparkline: SPARKLINES[1] },
    { label: 'Pending Review',   value: props.stats?.pending_review   ?? 0, icon: ICONS.eye,       iconBg: 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400',         valueClass: 'text-amber-600 dark:text-amber-400',     accentColor: '#f59e0b', sparkline: SPARKLINES[2] },
    { label: 'Anggota Tim',      value: props.stats?.team_members     ?? 0, icon: ICONS.users,     iconBg: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400', valueClass: 'text-ink-900 dark:text-white',            accentColor: '#10b981', sparkline: SPARKLINES[3] },
])
