                        ...
                        >
                            <div v-if="showCreateModal" class="bg-white dark:bg-ink-800 rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                                ...
                                <form @submit.prevent="submitCreate" class="px-6 py-5 space-y-4">
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        <div>
                                            <label class="block text-sm font-medium text-ink-700 dark:text-sage-300 mb-1.5">Start Date</label>
                                            <input v-model="createForm.start_date" type="date" class="w-full border border-ink-200 dark:border-ink-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-ink-850 text-ink-900 dark:text-sage-200 focus:outline-none focus:ring-2 focus:ring-ink-900/15 dark:focus:ring-sage-300/20 transition" />
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-ink-700 dark:text-sage-300 mb-1.5">End Date</label>
                                            <input v-model="createForm.end_date" type="date" class="w-full border border-ink-200 dark:border-ink-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-ink-850 text-ink-900 dark:text-sage-200 focus:outline-none focus:ring-2 focus:ring-ink-900/15 dark:focus:ring-sage-300/20 transition" />
                                        </div>
                                    </div>
            ...
            <div class="bg-white dark:bg-ink-800 rounded-xl border border-ink-900/8 dark:border-sage-300/8 p-4 shadow-sm">
                <div class="flex flex-wrap gap-3 items-center">
                    ...
                    <div class="flex items-center gap-2 border border-ink-200 dark:border-ink-600 bg-sage-50 dark:bg-ink-850 rounded-lg px-3 py-2">
                        <span class="text-xs text-ink-400 dark:text-sage-500 shrink-0">Start</span>
                        <input v-model="form.start_date" type="date"
                            class="w-[118px] bg-transparent text-xs text-ink-900 dark:text-sage-200 focus:outline-none dark:[color-scheme:dark]" />
                        <span class="text-ink-300 dark:text-sage-600 text-xs shrink-0">–</span>
                        <span class="text-xs text-ink-400 dark:text-sage-500 shrink-0">End</span>
                        <input v-model="form.end_date" type="date"
                            class="w-[118px] bg-transparent text-xs text-ink-900 dark:text-sage-200 focus:outline-none dark:[color-scheme:dark]" />
                        <button @click="applyDateFilter" title="Terapkan filter tanggal"
                            class="shrink-0 text-ink-400 dark:text-sage-500 hover:text-ink-700 dark:hover:text-sage-300 transition ml-1">
                        ...
                        <table class="w-full text-sm min-w-[700px]">
                            <thead>
                                <tr class="border-b border-ink-100 dark:border-sage-300/8 bg-sage-50 dark:bg-ink-850">
                                    <th class="text-left px-5 py-3 text-xs font-semibold text-ink-400 dark:text-sage-500 uppercase tracking-wide">
                                        <button @click="setSort('title')" class="flex items-center gap-1 hover:text-ink-600 dark:hover:text-sage-300 transition">
                                            Task <span :class="form.sort_by === 'title' ? 'text-ink-700 dark:text-sage-300' : 'opacity-40'">{{ sortIcon('title') }}</span>
                                        </button>
                                    </th>
                                    ...
                                    <th class="text-left px-4 py-3 text-xs font-semibold text-ink-400 dark:text-sage-500 uppercase tracking-wide">
                                        <button @click="setSort('priority')" class="flex items-center gap-1 hover:text-ink-600 dark:hover:text-sage-300 transition">
                                            Priority <span :class="form.sort_by === 'priority' ? 'text-ink-700 dark:text-sage-300' : 'opacity-40'">{{ sortIcon('priority') }}</span>
                                        </button>
                                    </th>
                                    <th class="text-left px-4 py-3 text-xs font-semibold text-ink-400 dark:text-sage-500 uppercase tracking-wide">
                                        <button @click="setSort('status')" class="flex items-center gap-1 hover:text-ink-600 dark:hover:text-sage-300 transition">
                                            Status <span :class="form.sort_by === 'status' ? 'text-ink-700 dark:text-sage-300' : 'opacity-40'">{{ sortIcon('status') }}</span>
                                        </button>
                                    </th>
                                    <th class="text-left px-4 py-3 text-xs font-semibold text-ink-400 dark:text-sage-500 uppercase tracking-wide">
                                        <button @click="setSort('due_date')" class="flex items-center gap-1 hover:text-ink-600 dark:hover:text-sage-300 transition">
                                            Deadline <span :class="form.sort_by === 'due_date' ? 'text-ink-700 dark:text-sage-300' : 'opacity-40'">{{ sortIcon('due_date') }}</span>
                                        </button>
                                    </th>
                                </tr>
                            </thead>
