<template>
    <AppLayout>
        ...
        <!-- ── Deadline Alert ──────────────────────────────────────── -->
        <div v-if="task.deadline_status === 'overdue'"
            class="flex items-center gap-3.5 px-5 py-4 bg-red-50 dark:bg-red-400/8 border border-red-200 dark:border-red-400/20 rounded-2xl mb-6">
            <svg class="w-5 h-5 text-red-500 dark:text-red-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
            </svg>
            <div>
                <p class="text-sm font-semibold text-red-700 dark:text-red-400">Task ini sudah melewati deadline!</p>
                <p class="text-xs text-red-600/70 dark:text-red-400/60 mt-0.5">Deadline: {{ formatDate(task.due_date) }}</p>
            </div>
        </div>
        <div v-else-if="task.deadline_status === 'warning'"
            class="flex items-center gap-3.5 px-5 py-4 bg-amber-50 dark:bg-amber-400/8 border border-amber-200 dark:border-amber-400/20 rounded-2xl mb-6">
            ...
            <div>
                <p class="text-sm font-semibold text-amber-700 dark:text-amber-400">Deadline mendekat!</p>
                <p class="text-xs text-amber-600/70 dark:text-amber-400/60 mt-0.5">Due {{ formatDate(task.due_date) }} — segera selesaikan.</p>
            </div>
        </div>
